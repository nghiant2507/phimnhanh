...
import { Clock, MoveRight, Play, Star } from 'lucide-react';
...
export const ListView = (props: ListViewProps) => {
  const { data, title, icon, numberItems = 15 } = props;
      ...
      <div className={'flex items-center justify-between'}>
        <h3 className="flex items-center gap-2 text-2xl font-bold border-b-4 border-primary pb-2 w-fit">
          ...
          <p className="font-montserrat">
            {title ? title : data?.data?.titlePage}
          </p>
        </h3>
        <Link
          href={data?.data?.seoOnPage?.og_url}
          className={
            'flex items-center gap-2 transition-all duration-300 bg-primary px-2 py-1.5 text-sm rounded-md hover:'
          }
        >
          Xem thêm
          <MoveRight size={16} />
